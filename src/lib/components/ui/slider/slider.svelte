...
>
	{#snippet children({ thumbItems })}
		<span
			...
			class={cn(
				"bg-muted rounded-full relative grow overflow-hidden data-horizontal:h-1 data-horizontal:w-full data-vertical:h-full data-vertical:w-1"
			)}
		>
		...
		{#each thumbItems as thumb (thumb)}
			<SliderPrimitive.Thumb
				...
				index={thumb.index}
				aria-label={typeof thumbAriaLabel === "function"
					? thumbAriaLabel(thumb.index)
					: thumbAriaLabel}
