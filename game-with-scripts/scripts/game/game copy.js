...
class Grid {
  ...
  enrich(grid, bonusGrid) {
    ...
    for (let x = 0; x < 15; ++x) {
      ...
      for (let y = 0; y < 15; ++y) {
        ...
        vInfo.valid = false;
        for (let i = 0; i < 26; ++i) {
          if (vInfo.top.length + vInfo.bottom.length === 0 || DB.has(vInfo.top + String.fromCharCode(i + A_CODE) + vInfo.bottom)) {
    ...
    for (let i = 1; i < amount; ++i) {
      ...
      if (blanks.length !== 0) {
        ...
        for (let k = comb.length - 1; k >= 0; --k) {
          ...
          let index = 0;
          add(rest.replace(/\?/g, () => blankValues.charAt(index++)), blankValues);
        }
      } else {
        add(rest, []);
      }
  ...
  solve(letters) {
    ...
    this.solveHorizontal(results, this.grid, allCombinations, maxSpotCount, false);
    this.solveHorizontal(results, this.invertedGrid, allCombinations, maxSpotCount, true);
    return results;
  }

  solveHorizontal(results, grid, allCombinations, maxSpotCount, inverted) {
    for (let y = 0; y < 15; ++y) {
      for (let x = 0; x < 15; ++x) {
        const tile = grid[y][x];
        ...
        for (let spotCount = 0; spotCount !== maxSpotCount; ++currentX) {
          ...
          if (!validSpot) validSpot = nextTile.top.length !== 0 || nextTile.bottom.length !== 0 || nextTile.right.length !== 0;
  ...
  checkHorizontalWord(word, start, letters, end, x, y, extra) {
    // if (word.toUpperCase() === 'ABJURAMES') {
    //   console.log('FOUND', start, end, letters, '*', x, y, extra)
    //   for (let i = letters.length - 1; i >= 0; --i) {
    //     const cp = letters[i];
    //     if (word.charCodeAt(cp[0]) !== cp[1]) console.log('NOPE!!');
    //   }
    // }
  ...
  checkVerticalWords(grid, word, x, y, start, end, spotCount, extraLetters) {
    for (let index = word.length - end - 1; index >= start; --index) {
      const tile = grid[y][x + index];
      if (tile.empty === true &&
        !(tile.validWordWith & (1 << (word.charCodeAt(index) - A_CODE))) // instead of tile.validWordWith.has(word.charCodeAt(index)) === false
      ) {
        return false;
      }
    }
  ...
  matchingWordsPerSpot(results, grid, x, y, combinations, spotCount, inverted, tile, nextTile, patternLetters, extraLetters) {
    ...
    for (let k = combi.length - 1; k >= 0; --k) {
      ...
      for (let l = words.length - 1; l >= 0; --l) {
        ...
        if ((hValid === true || this.checkHorizontalWord(word, tile.left, patternLetters, nextTile.right, y, wordX, extraLetters) === true)
...
// const results = newGrid.solve('OSTAOOI')
const results = newGrid.solve('MEUTIEZ')
