...
// Attention aux defs qui finissent par '...'
// Check thallium
// Check couleurs avec #abcdefg
// [OK] Check intuitionisme pour 2ème def
// Check Note :
...
(async function () {
  ...
  for (const letter of letters) {
    ...
    // const files = ['tourmaline.html.txt'];
    ...
    await PromisePool
      ...
      .process(async (file) => {
        ...
        if (word.length <= 15) {
          ...
          let wikWikDefInfo = def ? getOdsInfo(def) : { def: '' }
          def = def ? wikWikDefInfo.full() : '';

          if (def.endsWith('…')) {
            const tmp = (await getLongerDef(wikWikDefInfo.originalWord, wikWikDefInfo.def));
            if (tmp.broken || tmp.missing) {
              ++incompleteDefs;
            } else {
              ++fullDefFound;
            }
            wikWikDefInfo.def = tmp.def
            def = wikWikDefInfo.full();
          }

          if ($('h4:contains(WikWik.org) + ul li:first').text().trim() !== def) ++changed

          let odsInfo = wikWikDefInfo // def ? getOdsInfo(def) : { def: '' }
  ...
  db = Object.keys(db).reduce((h, k) => {
    h[k.toUpperCase()] = db[k];
    return h
  }, {});

  let content = "function om(o) { let m = new Map(), k = Object.keys(o); for (let i = k.length - 1; i >= 0; --i) m.set(k[i], o[k[i]]); return m; };\n";

  // Convert to a Map and save it
  fs.writeFileSync(join(DIR, '1mot-map.js'), content + "export default om(" + JSON.stringify(db, null, 2) + ")");
  // try {
