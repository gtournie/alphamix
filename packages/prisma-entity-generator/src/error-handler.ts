...
import { PrismaEntityGeneratorOptions } from './generator'
...
export const handleGenerateError = (e: Error) => {
	if (e instanceof GeneratorFormatNotValidError) {
		const options = Object.keys(PrismaEntityGeneratorOptions).map((key) => {
			const value = PrismaEntityGeneratorOptions[key]
