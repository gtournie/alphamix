...
export const PrismaEntityGeneratorOptions = {
	makeIndexFile: {
...
export type PrismaEntityGeneratorOptionsKeys =
	keyof typeof PrismaEntityGeneratorOptions
export type PrismaEntityGeneratorConfig = Partial<
	Record<PrismaEntityGeneratorOptionsKeys, any>
>

export class PrismaEntityGenerator {
	...
	static getInstance(options?: GeneratorOptions) {
		if (PrismaEntityGenerator.instance) {
			return PrismaEntityGenerator.instance
		}
		PrismaEntityGenerator.instance = new PrismaEntityGenerator(options)
		return PrismaEntityGenerator.instance
	}
	...
	run = async (): Promise<void> => {
		...
		files.forEach((fileRow) => {
			fileRow.imports.push(new ImportComponent(
				getRelativeTSPath(indexFilePath, indexFilePath),
				"Entity", // TODO
			))
		...
		if (config.makeIndexFile) {
			...
			const content = INDEX_TEMPLATE.replace(
				...
				.replace(
					'#!{RE_EXPORT_CLASSES}',
					files
						.map((f) => `	${f.prismaClass.reExportPrefixed('_')}`) // TODO: no need prefix now
						// .map((f) => f.prismaClass.name)
						.join('\r\n'),
				)
				.replace(
					'#!{CLASSES}',
	...
	getConfig = (): PrismaEntityGeneratorConfig => {
		...
		const result: PrismaEntityGeneratorConfig = {}
		for (const optionName in PrismaEntityGeneratorOptions) {
			const { defaultValue } = PrismaEntityGeneratorOptions[optionName]
