<script module>
	...
	const timezones = [
		...
		{
			value: "Europe",
			items: ["(GMT+0) London", "(GMT+1) Paris", "(GMT+1) Berlin", "(GMT+1) Rome", "(GMT+1) Madrid"],
		},
		{
			value: "Asia/Pacific",
			items: ["(GMT+9) Tokyo", "(GMT+8) Shanghai", "(GMT+8) Singapore", "(GMT+4) Dubai", "(GMT+11) Sydney"],
		},
	];
	...
	const { Story } = defineMeta({
		...
		parameters: {
			docs: {
				description: {
					component: "Autocomplete input built from Popover + Command primitives.",
				},
			},
		},
	});
...
<script>
	...
	let autoHighlight = $state("");

	let popupOpen = $state(false);
	let popupValue = $state(/** @type {typeof countries[0] | null} */ (null));
...
<Story name="Basic">
	{#snippet template()}
		<Popover.Root bind:open={basicOpen}>
			<Popover.Trigger>
				{#snippet child({ props })}
					<Button variant="outline" role="combobox" class="w-52 justify-between" {...props}>
					...
					<Command.List>
						...
						<Command.Group>
							{#each frameworks as fw}
								<Command.Item value={fw} onSelect={(v) => { basicValue = v === basicValue ? "" : v; basicOpen = false; }}>
									<CheckIcon class={cn("mr-2 size-4", basicValue === fw ? "opacity-100" : "opacity-0")} />
									{fw}
								</Command.Item>
							{/each}
						</Command.Group>
	...
	{#snippet template()}
		<Popover.Root bind:open={multipleOpen}>
			<Popover.Trigger>
				{#snippet child({ props })}
					<Button variant="outline" role="combobox" class="w-52 justify-between" {...props}>
						{multipleValues.length > 0 ? `${multipleValues.length} selected` : "Select frameworks"}
						<ChevronsUpDownIcon class="ml-2 size-4 shrink-0 opacity-50" />
					</Button>
				{/snippet}
			</Popover.Trigger>
						...
						<Command.Group>
							{#each frameworks as fw}
								<Command.Item value={fw} onSelect={() => toggleMultiple(fw)}>
									<CheckIcon class={cn("mr-2 size-4", multipleValues.includes(fw) ? "opacity-100" : "opacity-0")} />
									{fw}
								</Command.Item>
							{/each}
						</Command.Group>
			...
			<Popover.Content class="w-52 p-0">
				<Command.Root>
					...
					<Command.List>
						...
						<Command.Group>
							{#each frameworks as fw}
								<Command.Item value={fw} onSelect={(v) => { clearValue = v === clearValue ? "" : v; clearOpen = false; }}>
									<CheckIcon class={cn("mr-2 size-4", clearValue === fw ? "opacity-100" : "opacity-0")} />
									{fw}
								</Command.Item>
							{/each}
						</Command.Group>
...
<Story name="Groups">
	{#snippet template()}
		<Popover.Root bind:open={groupsOpen}>
			<Popover.Trigger>
				{#snippet child({ props })}
					<Button variant="outline" role="combobox" class="w-60 justify-between" {...props}>
						...
						{#each timezones as group}
							<Command.Group heading={group.value}>
								{#each group.items as tz}
									<Command.Item value={tz} onSelect={(v) => { groupsValue = v === groupsValue ? "" : v; groupsOpen = false; }}>
										<CheckIcon class={cn("mr-2 size-4", groupsValue === tz ? "opacity-100" : "opacity-0")} />
										{tz}
									</Command.Item>
								{/each}
							</Command.Group>
...
<Story name="Custom Items">
	{#snippet template()}
		<Popover.Root bind:open={customOpen}>
			<Popover.Trigger>
				{#snippet child({ props })}
					<Button variant="outline" role="combobox" class="w-64 justify-between font-normal" {...props}>
					...
					<Command.List>
						...
						<Command.Group>
							{#each countries as country}
								<Command.Item value={country.label} onSelect={(v) => { customValue = v === customValue ? "" : v; customOpen = false; }}>
									<CheckIcon class={cn("mr-2 size-4", customValue === country.label ? "opacity-100" : "opacity-0")} />
									<span class="flex flex-col">
										<span class="text-sm">{country.label}</span>
										<span class="text-muted-foreground text-xs">{country.continent} ({country.code.toUpperCase()})</span>
									</span>
								</Command.Item>
							{/each}
						</Command.Group>
...
<Story name="Invalid">
	{#snippet template()}
		<Field.Field data-invalid="true">
			<Field.Label for="combobox-invalid">Framework</Field.Label>
				...
				<Popover.Content class="w-52 p-0">
					<Command.Root>
						<Command.List>
							...
							<Command.Group>
								{#each frameworks as fw}
									<Command.Item value={fw} onSelect={(v) => { invalidValue = v === invalidValue ? "" : v; invalidOpen = false; }}>
										<CheckIcon class={cn("mr-2 size-4", invalidValue === fw ? "opacity-100" : "opacity-0")} />
										{fw}
									</Command.Item>
								{/each}
							</Command.Group>
			...
			<Field.Error>Please select a valid framework.</Field.Error>
		</Field.Field>
	{/snippet}
</Story>
...
<Story name="Disabled">
	{#snippet template()}
		<Popover.Root bind:open={disabledOpen}>
			<Popover.Trigger>
				{#snippet child({ props })}
					<Button variant="outline" role="combobox" class="w-52 justify-between" {...props} disabled>
			...
			<Popover.Content class="w-52 p-0">
				<Command.Root>
					...
					<Command.List>
						...
						<Command.Group>
							{#each frameworks as fw}
								<Command.Item value={fw}>{fw}</Command.Item>
							{/each}
						</Command.Group>
					</Command.List>
				</Command.Root>
...
<Story name="Auto Highlight">
	{#snippet template()}
		<Popover.Root bind:open={autoOpen} onOpenChange={(open) => { if (open) autoHighlight = autoSelected; }}>
			<Popover.Trigger>
				{#snippet child({ props })}
					<Button variant="outline" role="combobox" class="w-52 justify-between" {...props}>
			...
			<Popover.Content class="w-52 p-0">
				<Command.Root bind:value={autoHighlight}>
					...
					<Command.List>
						...
						<Command.Group>
							{#each frameworks as fw}
								<Command.Item value={fw} onSelect={(v) => { autoSelected = v === autoSelected ? "" : v; autoOpen = false; }}>
									<CheckIcon class={cn("mr-2 size-4", autoSelected === fw ? "opacity-100" : "opacity-0")} />
									{fw}
								</Command.Item>
							{/each}
						</Command.Group>
...
<Story name="Popup">
	{#snippet template()}
		<Popover.Root bind:open={popupOpen}>
			<Popover.Trigger>
				{#snippet child({ props })}
					<Button variant="outline" class="w-64 justify-between font-normal" role="combobox" {...props}>
						{popupValue?.label ?? "Select a country"}
						<ChevronsUpDownIcon class="ml-2 size-4 shrink-0 opacity-50" />
					</Button>
				{/snippet}
			</Popover.Trigger>
			<Popover.Content class="w-64 p-0">
				<Command.Root>
					...
					<Command.List>
						...
						<Command.Group>
							{#each countries as country}
								<Command.Item value={country.label} onSelect={() => { popupValue = country; popupOpen = false; }}>
									<CheckIcon class={cn("mr-2 size-4", popupValue?.code === country.code ? "opacity-100" : "opacity-0")} />
									{country.label}
								</Command.Item>
							{/each}
						</Command.Group>
				...
				{#snippet child({ props })}
					<InputGroup.Root>
						...
						<Button
							variant="outline"
							role="combobox"
							class="w-60 justify-between rounded-l-none border-l-0 font-normal"
							{...props}
						>
				...
				<Command.Root>
					...
					<Command.List>
						...
						{#each timezones as group}
							<Command.Group heading={group.value}>
								{#each group.items as tz}
									<Command.Item value={tz} onSelect={(v) => { addonValue = v === addonValue ? "" : v; addonOpen = false; }}>
										<CheckIcon class={cn("mr-2 size-4", addonValue === tz ? "opacity-100" : "opacity-0")} />
										{tz}
									</Command.Item>
								{/each}
							</Command.Group>
