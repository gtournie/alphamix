	...
	{#snippet template()}
		...
		<div class="flex flex-wrap gap-2">
			...
			<Button variant="outline" onclick={() => toast.success("Profile updated")}
				>Success</Button
			>
			<Button variant="outline" onclick={() => toast.info("Update available")}
				>Info</Button
			>
			<Button
				variant="outline"
				onclick={() => toast.warning("Caution required")}>Warning</Button
			>
			<Button
				variant="outline"
				onclick={() => toast.error("Something went wrong")}>Error</Button
			>
		</div>
	{/snippet}
</Story>
