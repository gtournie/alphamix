					...
					<Calendar.Grid>
						<Calendar.GridHead>
							<Calendar.GridRow class="select-none">
								{#if showWeekNumber}
									<Calendar.HeadCell class="size-(--cell-size)"></Calendar.HeadCell>
								{/if}
						...
						<Calendar.GridBody>
							{#each month.weeks as weekDates (weekDates)}
								...
								<Calendar.GridRow class="mt-2 w-full">
									{#if showWeekNumber}
										<td class="relative size-(--cell-size) p-0 text-center text-sm"><span class="text-muted-foreground/50 flex size-(--cell-size) items-center justify-center p-0 text-xs tabular-nums">{String(weekNum).padStart(2, "0")}</span></td>
									{/if}
