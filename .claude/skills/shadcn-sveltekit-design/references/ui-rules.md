...
The project enforces a11y via axe-core in `bun run test` (`a11y.test: 'error'`). Every piece of UI you produce must pass. Apply these rules proactively:

- **Icon-only buttons** must have `aria-label`.
- **Trigger wrappers** (`AlertDialog.Trigger`, `Dialog.Trigger`, etc.) must use `{#snippet child({ props })}` to avoid nested interactive elements (button-in-button).
- **Slider**: always pass `thumbAriaLabel` (string or `(index) => string`).
- **Progress**: always set `aria-label` or `aria-labelledby`.
- **Form controls outside a `<label>`** (Switch, Checkbox, NativeSelect, InputOTP): must have `aria-label` or `aria-labelledby`.
- **Combobox triggers**: the Button/div with `role="combobox"` must have `aria-label`.
- **`text-muted-foreground` on `bg-muted`**: never reduce opacity (e.g. `/50`) — the base token is already tuned to 4.5:1.
- **`<dl>` elements**: only `<div>`, `<dt>`, `<dd>` as direct children — no `<Separator>`.
- **Empty `<th>`** (e.g. checkbox column): add `<span class="sr-only">…</span>`.
- **`aria-label` on `<span>`/`<div>`**: requires an explicit `role` (e.g. `role="img"`).
